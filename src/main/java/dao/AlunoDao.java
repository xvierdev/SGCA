...
import java.sql.Date; // Para conversão de LocalDate para java.sql.Date
...
public class AlunoDao {
    ...
    public AlunoDao() throws SQLException { // Construtor deve lançar SQLException
     ...
     * Inclui validação se o curso está cheio ou inativo.
     ...
     * @throws IllegalStateException Se o curso estiver cheio ou inativo.
     */
    public Aluno adicionar(Aluno aluno) throws SQLException, IllegalStateException {
        // Validações de negócio que dependem de outro DAO
        // Instancia CursoDao
        try (CursoDao cursoDao = new CursoDao()) { // Usar try-with-resources para garantir que o CursoDao seja fechado
            // 1. Verificar se o curso existe e está ativo
            if (!cursoDao.estaAtivo(aluno.getIdCurso())) {
                throw new IllegalStateException("O curso com ID " + aluno.getIdCurso() + " não está ativo ou não existe.");
            }
            // 2. Verificar se o curso está cheio
            if (cursoDao.cursoCheio(aluno.getIdCurso())) {
                throw new IllegalStateException("O curso com ID " + aluno.getIdCurso() + " está lotado. Não é possível adicionar mais alunos.");
            }
        } // cursoDao.close() será chamado automaticamente aqui
     ...
     * @param cpf O CPF do aluno a ser removido.
     ...
     * @return true se o aluno está ativo, false caso contrário (incluindo se não existir).
     * @throws SQLException Se ocorrer um erro de acesso ao banco de dados.
     */
